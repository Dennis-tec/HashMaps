...
/*
 * Map implementation using hash table with double probing.
 *
 * @author Eric Zamore
...
public class DoubleHashMap<K,V> extends AbstractHashMap<K,V> {
  ...
  private int maxProbes;   // max number of probes during insertion
  private int totalProbes; // totoal number of probes during insertion
  ...
  private int findSlot(int h, K k) {
    ...
    do {
      ...
      probes++;
      j = (j + f(probes, k)) % capacity;                       // keep looking (cyclically)
  ...
  /**
   * Use to compute double hashing on collision.
   * @param i - number of collision for current number
   * @param k - key of an entry
   * @return - returns index to add a new entry
   */
  public int f(int i, K k) {
    int Q = 1000001;
    int h_k = Q - (k.hashCode() % Q);
    return i * h_k;
  }
  /**
  ...
  /**
   * Compute and return the average number of probes during insertion.
   * @return - double number of probes
   */
  public double getAvgProbes() {
    return (double) totalProbes / (double) insertions;
  }

  /**
   * Compute and return the average number of probes during insertion.
   * @return - int max number of probes
   */
  ...
  /**
   * Compute and get the load factor for a HashMap.
   * @return - get the load
   */
